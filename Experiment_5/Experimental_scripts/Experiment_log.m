...
if exist('Experiment_log.csv', 'file') ~= 2
    ...
    string = 'Output,Start-time,End-time,Study-type|Prime-type|Calibration-type,Initials,Notes\n';
    fprintf(fid,string);
    fclose(fid);
end
...
curDesign = [ num2str( StudyType ) '|' num2str( PrimeVer ) '|' .... 
    num2str( CalType ) ];

% Output Start-time End-time Initials Notes
fid = fopen('Experiment_log.csv', 'at'); % Open for writing
string = [ csvOutputFile '-' matOutputFile ',' ...
     stTime ',' edTime ',' curDesign ',' answer{1} ...
    ',' answer{2} char(10) ];
