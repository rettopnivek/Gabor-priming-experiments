function [ gabor ] = Overlaid_Gabors( imSize, lambda, amp, rho, theta, sigma, show, prp, step )
% Purpose:
...
hght = max( max( grating ) );        % Determine height of lower/upper bounds
grating = (1 + grating)/2;           % Shift to [0,1] range
if step == 1
    grating = round( grating );      % Convert to step function
    grating = 2*grating - 1;         % Shift to [-1,1] range
    grating = grating*hght;          % Adjust amplitude
end
grating = (1 + grating)/2;           % Shift to [0,1] range
...
hght = max( max( grating2 ) );         % Determine height of lower/upper bounds
grating2 = (1 + grating2)/2;           % Shift to [0,1] range
if step == 1
    grating2 = round( grating2 );      % Convert to step function
    grating2 = grating2*2 - 1;         % Shift to [-1,1] range
    grating2 = grating2*hght;          % Adjust amplitude
end
grating2 = (1 + grating2)/2;           % Shift to [0,1] range

% Create grid by overlaying gabors
